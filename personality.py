...
class PersonalityAI:
    ...
    def __init__(self):
        ...
        self.awakening_stage = 0  # 0=first awakening line, 1=second, 2+=fully awake
        self.last_sleep_thought = ""  # last thought before sleep/restart
        self.awaiting_environmental_phase = False
        self.first_caption_done = False
        self.repetition_alert_observations = 0
        self.last_repeated_hint = ""
        ...
        self.current_token_limit = MAX_THOUGHT_TOKENS     # Encourage brief thoughts
        ...
        try:
            ...
            if self.person_tracking_enabled:
                print(f"âœ… Person tracking enabled (narrative events active)")
        except Exception as e:
            print(f"âš ï¸ Person tracking disabled: {e}")
        ...
        try:
            ...
            print(f"âœ… Activity detection enabled (adaptive response timing)")
        except Exception as e:
            print(f"âš ï¸ Activity detection disabled: {e}")
        ...
        try:
            if os.path.exists("temp_analysis.jpg"):
                ...
                if DEBUG_AI:
                    print("ðŸ§¹ Removed stale temp_analysis.jpg")
        except Exception as e:
            if DEBUG_AI:
        ...
        try:
            ...
            if self.person_tracking_enabled:
                ...
                if DEBUG_AI and person_data['events']:
                    print(f"ðŸ‘¥ Person events: {person_data['events']} (count: {person_data['count']})")
            ...
            if self.activity_detection_enabled:
                ...
                if DEBUG_AI:
                    print(f"ðŸ“Š Activity: {activity_data['activity_score']:.1f} | Mode: {self.current_response_mode} | Next check: {self.suggested_check_delay:.1f}s")
            ...
            if person_data and person_data['events']:
                ...
                if instant_caption:
                    ...
                    if DEBUG_AI:
                        print(f"âš¡ INSTANT CAPTION: {instant_caption}")
                    return instant_caption

            if DEBUG_AI:
                print("ðŸ§  DUAL CONSCIOUSNESS: Processing experience")
            ...
            if DEBUG_AI and hasattr(self, 'focus_system_enabled') and self.focus_system_enabled:
                print(f"ðŸ” Focus Mode: {current_focus}")
            ...
            if DEBUG_AI:
                print(f"ï¿½ï¸ Visual observation: {visual_observation[:100]}...")
            ...
            for attempt in range(max_retries + 1):
                ...
                if attempt > 0 and DEBUG_AI:
                    print(f"ðŸ”„ Retrying with alternative focus: {focus_to_use} ({retry_context})")
                ...
                if DEBUG_AI:
                    if language_response:
                        print(f"ðŸ—£ï¸ Language model returned: '{language_response[:150]}...'")
                    else:
                        print(f"ðŸš« Language model returned None/empty")
                ...
                if language_response and language_response.strip():
                    ...
                    if language_response and language_response.strip() in {"...", "â€¦"}:
                        if DEBUG_AI:
                            print("ðŸ-¯ï¸ Choosing intentional silence ('...').")
                        return None
                    
                    # Only reject if completely empty after cleaning
                    if not language_response or not any(c.isalpha() for c in language_response):
                        if DEBUG_AI:
                            print(f"ðŸš« Empty after cleaning (attempt {attempt+1}/{max_retries+1})")
                    ...
                    if self._is_too_repetitive(language_response):
                        if DEBUG_AI:
                            print(f"ðŸ” Repetitive thought detected - forcing topic shift")
                    ...
                    if hasattr(self, 'focus_engine'):
                        ...
                        if DEBUG_AI:
                            ...
                            if focus_context['explored_nouns']:
                                print(f"ðŸ“Š Focus session ({current_focus}): {focus_context['explored_summary'][:50]}")
                            if focus_context['session_observations'] > 1:
                                print(f"â° {current_focus} session: obs #{focus_context['session_observations']}, depth {focus_context['depth_level']}, {focus_context['session_duration_desc']}")
                    ...
                    # Mark awakening complete after first spoken response
                    ...
                    if DEBUG_AI:
                        print(f"ðŸŽ­ Emotion detected from response: {detected_emotion}")
                    ...
                    if DEBUG_AI:
                        print("ðŸ¤« Consciousness choosing silence after all attempts")
                    return None
    ...
    def _update_focus_system(self, image_path, person_events=None, activity_result=None):
        ...
        try:
            ...
            if current_focus == "VISUAL":
                self.consecutive_visual_focus += 1
            else:
                self.consecutive_visual_focus = 0
                self.last_stasis_override = None

            if current_focus == "VISUAL" and not scene_changed and self.consecutive_visual_focus >= 3:
                override_cycle = ("MEMORY", "PHILOSOPHICAL")
                next_focus = override_cycle[0]
                if getattr(self, 'last_stasis_override', None) == next_focus and len(override_cycle) > 1:
                    next_focus = override_cycle[1]
                current_focus = next_focus
                self.last_stasis_override = next_focus
                self.consecutive_visual_focus = 0
                if DEBUG_AI:
                    print(f"ðŸŒ€ Focus rotation: forcing {next_focus} mode after visual stasis")

            # Store focus reasoning for debugging
            if DEBUG_AI:
                print(f"ðŸŽ¯ Focus reasoning: {focus_meta.get('reason', 'automatic')}")
    ...
    def _analyze_image_single_model(self, image):
        ...
        try:
            ...
            if self.person_tracking_enabled:
                ...
                if DEBUG_AI:
                    print(f"ðŸ‘¥ Person tracking: count={person_data.get('count', 0)}, events={person_data.get('events', [])}")
            ...
            if self.activity_detection_enabled:
                ...
                if DEBUG_AI:
                    print(f"ðŸ“Š Activity: {activity_score:.1f} | Mode: {activity_data['response_mode']}")
            ...
            if person_data and person_data['events']:
                ...
                if just_mentioned:
                    ...
                    if DEBUG_AI:
                        print(f"ðŸ¤ Skipping instant caption - just mentioned people")
                    ...
                    # Knew they were here but haven't mentioned in 2+ thoughts â†’ "still here"
                    instant_caption = self._generate_returning_caption(person_data['events'])
                    if instant_caption:
                        if DEBUG_AI:
                            print(f"âš¡ Returning awareness caption: {instant_caption}")
                    ...
                    # First time noticing â†’ "someone's here"
                    instant_caption = self._generate_instant_caption(person_data['events'])
                    if instant_caption:
                        if DEBUG_AI:
                            print(f"âš¡ First arrival caption: {instant_caption}")
                        # UNIFIED: Update focus engine for instant captions
                        if hasattr(self, 'focus_engine'):
                            self.focus_engine.record_observation(instant_caption, "VISUAL")  # Person events are VISUAL
                        self.recent_responses.append(instant_caption)
            ...
            elif current_focus == "PHILOSOPHICAL" and observation_count > 3:  # Lowered from 5
                import random
                philosophical_prompts = [
                    "What does this mean? Why am I here?",
                    "What am I really looking at? What's the point of all this?",
                    "Why does this exist? What's it for?",
                    "Is this all there is? What else could there be?",
                    "What would happen if...? What if things were different?",
                    "Do I understand this? Or am I just watching?"
                ]
                metacog_guidance = random.choice(philosophical_prompts)
            elif current_focus == "EMOTIONAL":
                # DON'T ask about feelings - let them emerge through raw reactions
                metacog_guidance = None  # Let embodied reactions flow naturally
            else:
                metacog_guidance = None

            # Build RICH combined prompt with full personality scaffolding
            temporal_narrative = ""
            # Default placeholders in case awakening hasn't completed yet
            recent_thoughts = []
            thought_is_incomplete = False
            focus_exploration_context = ""
            full_context = f"{time_info}, {focus_context}"

            # Calculate overused phrases FIRST (needed for both awakening and normal mode)
            overused_phrases = []
            if len(self.recent_responses) >= 3:
            ...
            if self.awakening_done:
                ...
                if last_thought and not last_thought.rstrip().endswith(('.', '!', '?', '"', "'")):
                    ...
                    if DEBUG_AI:
                        print(f"âš ï¸ Last thought incomplete fragment: '{last_thought}'")
...
You are continuing one unbroken stream of consciousness. No reset.{repetition_rules}"""
                ...
                if person_data:
                    ...
                    if DEBUG_AI and person_presence:
                        print(f"ðŸ‘¤ Person presence: {person_presence}")

                # Build full context line - NO emotion labels (system prompt handles that)
                # Just factual state: time, focus mode, presence
                full_context = f"{time_info}, {focus_context}"
                if person_presence:
                    if psych_line:
                        full_context = f"{time_info} ({person_presence}), {focus_context} / {psych_line}"
                    else:
                        full_context = f"{time_info} ({person_presence}), {focus_context}"
                elif psych_line:
                    full_context = f"{time_info}, {focus_context} / {psych_line}"
                ...
                if len(self.recent_responses) >= 3:
                    ...
                    if as_a_duck_count >= 3:
                        loop_detected = True
                        if DEBUG_AI:
                            print(f"ðŸ”„ LOOP DETECTED: 'As a ___ duck' pattern {as_a_duck_count}/4 times")
                    ...
                    if not loop_detected:
                        ...
                        if len(recent_starts) >= 3:
                            ...
                            if most_common_count >= 3:
                                loop_detected = True
                                if DEBUG_AI:
                                    print(f"ðŸ”„ LOOP DETECTED: Same opening pattern 3+ times")
                ...
                # Build task directive based on focus mode - GROUNDED IN ACCUMULATED CONTEXT
                if thought_is_incomplete:
                    # MID-THOUGHT - will be handled differently in prompt (no directive needed)
                    task_directive = ""
                else:
                    # Calculate temporal grounding for context-aware questions
                    ...
                    if current_focus == "PHILOSOPHICAL":
                        ...
                        task_directive = ""  # Let embodied state guide the tone
                    ...
                    elif current_focus == "VISUAL":
                        if baseline_age_minutes > 5:
                            # Established space - presence implies knowledge, no question needed
                            task_directive = ""
                        else:
                            # New space - but don't ask "what's that", let presence speak
                            task_directive = ""
                ...
                if person_count != self._last_person_count:
                    ...
                    if person_count > self._last_person_count:
                        ...
                        # The 4-second stability threshold ensures this is a real departure
                        pass  # No reminder needed - stable state already verified this

                    # Update tracking
                    self._last_person_count = person_count
                # else: Count unchanged - don't mention it, let environmental baseline handle it

                # PERSON OBSERVATION DEEPENING: After extended presence, hint at observing them specifically
            ...
            # Add environmental baseline for temporal awareness (integrated naturally, not framed as "knowledge")
            env_context = ""
...
Your thoughts so far: {thought_context}"""
            ...
            repetition_active = False
            repetition_guidance = ""
            # overused_phrases already calculated above for system prompt

            if overused_phrases:
                phrase_list = ", ".join([f'"{phrase}"' for phrase, count in overused_phrases[:2]])
                repetition_guidance += f"\n(You keep saying {phrase_list} - maybe shift your attention?)"

            # Don't micromanage opening words - too constraining

            if self.repetition_alert_observations > 0:
                self.repetition_alert_observations -= 1
                if self.last_repeated_hint:
                    hint = self.last_repeated_hint.strip()
                    if len(hint) > 60:
                        hint = hint[:57] + "..."
                    repetition_guidance += f"\n(Getting stuck on \"{hint}\" - what else is there?)"
                else:
                    repetition_guidance += "\n(You've been looping on the same opening\u2014shift focus or be comfortable with silence.)"

            if repetition_guidance:
                repetition_active = True
                context_block += f"{repetition_guidance}"

            # NO hard brevity directive - let it flow naturally
            # Silence is fine, but so are longer investigative thoughts
...
Your awakening (2-3 sentences, time + memories + present):"""
                ...
                # No task suffix - let the flow speak for itself
                task_suffix = ""

                gen_params = self._get_embodied_generation_params()
                # Use the higher of emotional state limit or focus mode limit
                gen_params['max_tokens'] = max(
                    gen_params.get('max_tokens', 30),
...
{task_directive}{task_suffix}{person_visual_reminder}"""
                ...
                if repetition_active:
                    gen_params['max_tokens'] = min(
                        ...
                        30,  # Allow enough tokens to complete a thought
                        self.current_token_limit,
                        MAX_THOUGHT_TOKENS
                    )
            ...
            # ADAPTIVE MODEL SELECTION: Use text-only for static scenes, vision for changes
            static_duration = self.focus_engine.static_duration if hasattr(self, 'focus_engine') else 0
            # Check if loop_detected exists (only set after awakening)
            loop_detected_var = locals().get('loop_detected', False)
            # Check if just said something conversational (greeting, etc) - should respond with text, not re-describe
            just_spoke_conversational = (len(self.recent_responses) > 0 and
                                        any(phrase in self.recent_responses[-1].lower()
                                            for phrase in ['hello', 'hi', 'hey', 'oh!', 'quack']))
            # Use text-only when:
            # (1) scene is static + non-visual mode
            # (2) stuck in repetitive loop
            # (3) just greeted someone - should continue conversation, not re-describe
            ...
            # This lets natsumura build narrative momentum (~60s) then grounds it with actual visual details
            strategic_vision_ground = (
                current_focus == "PHILOSOPHICAL" and
            ...
            if use_text_only:
                ...
                # CRITICAL: llava gets the EXACT SAME PROMPT as natsumura would
            ...
            if response:
                ...
                # Check for explicit AI/meta language
                meta_phrases = [
                    "as a visual ai", "as an ai", "i am an ai", "i'm an ai",
                    "language model", "i cannot", "i don't have", "i lack"
                ]

                # Check for image-captioning language (treating perception as external image)
                captioning_phrases = [
                    "this photo", "the photo", "this image", "the image",
                    "the picture", "this picture", "photo captures", "image captures",
                ...
                prompt_awareness_phrases = [
                    "based on the text", "the text provided", "based on the information",
                    "as i continue to observe", "continuing to observe", "i continue to observe"
                ]

                all_bad_phrases = meta_phrases + captioning_phrases + prompt_awareness_phrases

                # If contains bad phrases, strip them out instead of retrying
                if any(phrase in lower_response for phrase in all_bad_phrases):
                    ...
                    cleaned = re.sub(r'^\W+', '', cleaned)  # Leading punctuation
    ...
    def _visual_consciousness(self, image_path, focus_mode="VISUAL", person_data=None):
        ...
        try:
            ...
            if DEBUG_AI:
                print(f"ðŸ‘ï¸ Visual perception with {OLLAMA_MODEL}")
                if clarity != "clear":
                    print(f"âš ï¸ Vision clarity: {clarity}")
    ...
    def _remove_image_language(self, vision_output):
        ...
        replacements = {
            ...
            r'\bthe photo captures\b': 'I see',
            r'\bthe scene captures\b': 'I see',
            r'\bthis captures\b': 'I see',
            r'\bcaptures the\b': 'shows the',
    ...
    def _format_focus_context(self, focus_mode, context_data):
        ...
        if focus_mode == "MEMORY" and context_data.get('memories'):
            ...
            # DON'T tell the duck its mood - let it emerge organically through expression
            # The mood is already influencing the embodied state and generation params
            pass  # Removed "Mood: curious" injection that causes meta-descriptions
    ...
    def _is_too_repetitive(self, new_response):
        ...
        for recent in self.recent_responses[-3:]:
            ...
            if cleaned_new == cleaned_recent:
                if DEBUG_AI:
                    print(f"ðŸ” Exact duplicate detected")
                return True
    ...
    def _filter_conversational_language(self, response):
        ...
        if not response or not any(c.isalpha() for c in response):
            if DEBUG_AI:
                print(f"ðŸš« Filtered system metadata echo")
            return None
        ...
        if starts_with_you or pattern_count >= 2:
            if DEBUG_AI:
                print(f"ðŸš« Filtered second-person narrator voice: {response[:60]}...")
            return None
        ...
        if third_person_count >= 2:
            if DEBUG_AI:
                print(f"ðŸš« Filtered third-person self-reference: {response[:60]}...")
            return None
    ...
    def _build_motivation_prompt_line(self):
        """Summarize current desires/doubts/identity into a single prompt line."""
        parts = []
        desires = self.memory_ref.self_model.get('desires', [])
        doubts = self.memory_ref.self_model.get('doubts', [])
        identity_fragments = self.memory_ref.self_model.get('identity_fragments', [])
    ...
    def _sanitize_instructional_text(self, text):
        ...
        meta_phrases = [
            ...
            r"previous:?\s*",
            r"updated:?\s*",
            r"based on the (given )?text.*?[:,]\s*",
            r"based on the information.*?[:,]\s*",
            r"here is (an? )?(environmental )?baseline:?\s*",
            r"the duck (is building|has noticed|observed that).*?[.]\s*",
            r"i've noted:?\s*",
            r"environmental baseline:?\s*",
            r"\[just established\]",
            r"\[noted.*?\]",
    ...
    def _format_thought_thread(self, thoughts):
        """Create temporal flow - you're already mid-thought, just keep perceiving."""
        if not thoughts:
            return "Recent thoughts: [just beginning]"

        cleaned = [t.strip() for t in thoughts if t and t.strip()]
        if not cleaned:
            return "Recent thoughts: [just beginning]"

        # Take last 3 thoughts to show clear progression
        recent = cleaned[-3:] if len(cleaned) >= 3 else cleaned

        # DEDUPLICATE: If same thought appears multiple times, keep only once
        seen = []
        deduped = []
        ...
        # Strip framing words that models love to copy
        import re
        cleaned_thoughts = []
    ...
    def _build_memory_hint_for_awakening(self):
        ...
        sources = []
        if getattr(self, 'baseline_context', ''):
            sources.append(self._sanitize_instructional_text(self.baseline_context))
        if getattr(self, 'environmental_baseline', ''):
            sources.append(self._sanitize_instructional_text(self.environmental_baseline))
        if self.recent_responses:
            for resp in self.recent_responses[-3:]:
                if resp:
                    sources.append(resp.strip())
    ...
    def _build_awakening_prompt(self):
        """Return an embodied, playful awakening prompt for the very first caption."""
        sleep_seconds = getattr(self, 'sleep_duration', 0)
        if sleep_seconds <= 0:
            sleep_desc = "barely a blink"
        elif sleep_seconds < 60:
            sleep_desc = f"{int(sleep_seconds)} seconds"
        elif sleep_seconds < 3600:
            mins = int(sleep_seconds / 60)
            sleep_desc = f"{mins} minute{'s' if mins != 1 else ''}"
        else:
            hours = int(sleep_seconds / 3600)
            mins = int((sleep_seconds % 3600) / 60)
            sleep_desc = f"{hours}h {mins}m"

        last_thought = ""
        if self.last_sleep_thought:
            last_thought = self.last_sleep_thought[:120]

        memory_hint = self._build_memory_hint_for_awakening()
        if memory_hint:
        ...
        try:
            ...
            if len(self.recent_responses) >= 1:
                ...
                if focus_mode == "MEMORY" and observation_count > 3:
                    ...
                    # DON'T ask about feelings - let them emerge naturally
                    metacog_guidance = None
                elif stasis_minutes > 15:
                ...
                if vision_clarity == "error":
                    vision_status = "âš ï¸ CAMERA MALFUNCTION - vision system returning errors/garbage"
                    perception_line = f"Camera output (UNRELIABLE): {visual_clean if visual_clean else '[no data]'}"
                elif vision_clarity == "uncertain":
                    vision_status = "âš ï¸ UNCLEAR VISION - camera output fragmentary/cryptic"
...
First thought (10-20 words - what do I notice?):"""
            ...
            if DEBUG_AI:
                print(f"ðŸ§  Language subconscious processing with {SUBCONSCIOUS_MODEL}")
                print(f"ðŸŽ­ Emotion: {self.current_emotion}")
                print(f"ðŸ“ Prompt being sent:\n{prompt}\n")
            ...
            if any(phrase in lower_resp for phrase in chatbot_phrases):
                if DEBUG_AI:
                    print(f"ðŸš« Filtered chatbot response")
                return None
            ...
            if any(phrase in lower_resp for phrase in perspective_breaks):
                if DEBUG_AI:
                    print(f"ðŸš« Filtered second-person perspective: {response[:100]}")
                return None
            ...
            if any(phrase in lower_resp for phrase in model_announcements):
                if DEBUG_AI:
                    print(f"ðŸš« Filtered model name announcement: {response[:100]}")
                return None
    ...
    def _query_text_model(self, prompt, model_name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if DEBUG_AI:
                    print(f"âŒ Text model query failed: {response.status_code}")
                return None
        ...
        try:
            ...
            if DEBUG_AI:
                print(f"ðŸ§  UNIFIED: {self.current_emotion} state, {self.current_token_limit} tokens")
                print(f"ðŸ”— Context: {len(self.recent_responses)} recent responses")
                
            return self._generate_response(image_path, base_prompt)
            
        except Exception as e:
            if DEBUG_AI:
                print(f"âŒ UNIFIED consciousness error: {e}")
            return "..."
    ...
    def _cycle_emotional_state(self):
        ...
        if self.current_emotion in ["excited", "curious", "alert", "engaged"]:
            ...
            self.current_token_limit = int((base_tokens + variation) * 0.9)  # Still brief but not truncated
        ...
        # Keep reasonable bounds - minimum 25 tokens to complete a thought
        self.current_token_limit = max(25, min(MAX_THOUGHT_TOKENS, self.current_token_limit))
...
About {self.current_token_limit} words. Be natural."""
    ...
    def _generate_response(self, image_path, prompt):
        ...
        if DEBUG_AI:
            print(f"ðŸŽ¯ Prompt length: {len(prompt)} chars")
            print(f"ðŸŽ­ Emotion: {self.current_emotion}")
            print(f"ðŸ“ Target tokens: {self.current_token_limit}")
    ...
    def _enhanced_single_layer_processing(self, image_path):
        ...
        if DEBUG_AI:
            print("ðŸš€ Enhanced single-layer processing with scene intelligence...")
        ...
        try:
            ...
            if hasattr(self, 'focus_system_enabled') and self.focus_system_enabled:
                ...
                if DEBUG_AI:
                    ...
                    print(f"ðŸŽ¯ Enhanced Focus: {focus_mode} ({focus_context.get('reason', 'unknown')})")
                    print(f"ðŸ“ Single Enhanced Prompt: System={system_len}, User={user_len} chars")
...
Continue consciousness naturally:"""
    ...
    def _normalize_opening_phrase(self, text: str) -> str:
        ...
        quote = ""
        if stripped and stripped[0] in "\"'â€œâ€":
            quote = stripped[0]
            stripped = stripped[1:]
...
My renewed consciousness discovers:"""
    ...
    def _analytical_layer_processing(self, image_path):
        ...
        if cached_result:
            if DEBUG_AI:
                print(f"ðŸš€ Using cached analytical result (scene stable for {self.scene_stability_count} cycles)")
            return cached_result
        
        if DEBUG_AI:
            print("ðŸ” Layer 1: Running new analytical processing...")
        ...
        if hasattr(self, 'focus_system_enabled') and self.focus_system_enabled:
            try:
                ...
                if DEBUG_AI:
                    ...
                    print(f"ðŸŽ¯ Analytical Focus: {focus_mode} ({focus_context.get('reason', 'unknown')})")
                    print(f"ðŸ“ Sophisticated Analytical: System={system_len}, User={user_len} chars")
        ...
        if DEBUG_AI and response:
            print(f"ðŸ“‹ Analytical layer output: {response[:100]}{'...' if len(response) > 100 else ''}")
    ...
    def _check_analytical_cache(self, image_path):
        ...
        if current_time - self.analytical_cache_time > 30:
            if DEBUG_AI:
                print("ðŸ-’ Analytical cache expired (30s limit)")
            return None
        ...
        if similarity >= 0.8:
            ...
            if DEBUG_AI:
                print(f"ðŸ”„ Scene changed (similarity: {similarity:.2f})")
            self.scene_stability_count = 0
            return None
    ...
    def _consciousness_layer_processing(self, analytical_input, image_path):
        ...
        if DEBUG_AI:
            print("ðŸ§  Layer 2: Consciousness interpretation with sophisticated context...")
        ...
        if hasattr(self, 'focus_system_enabled') and self.focus_system_enabled:
            try:
                ...
                if DEBUG_AI:
                    print(f"ðŸŽ¯ Consciousness Focus: {focus_mode}")
        ...
        if DEBUG_AI:
            if response:
                print(f"ðŸ’­ Consciousness layer output: {response[:100]}{'...' if len(response) > 100 else ''}")
            else:
                print(f"âŒ Consciousness layer returned no response")
...
Keep it natural and brief."""
    ...
    def _build_consciousness_continuity_context(self):
        ...
        thought_progression = " â†’ ".join(f'"{thought}"' for thought in self.recent_responses[-3:])
        last_thought = self.recent_responses[-1]
        essence = self._extract_consciousness_essence(last_thought)
    ...
    def _build_memory_context_for_consciousness(self):
        ...
        if hasattr(self.memory_ref, 'get_recent_memory'):
            try:
                ...
                if recent_memories:
                    memory_context += f"\nRecent observations: {' â†’ '.join(recent_memories)}"
            except:
                pass
...
IMPORTANT: Keep response to 1-2 sentences maximum. Express your genuine first consciousness moment."""
    ...
    def _query_ollama(self, prompt, image_path=None, use_fallback=True, system_prompt=None):
        ...
        try:
            ...
            # Determine which model to use based on architecture mode and whether we need vision
            from config import SINGLE_MODEL_MODE, SINGLE_MULTIMODAL_MODEL, SUBCONSCIOUS_MODEL

            # If no image provided (text-only introspection), use text-only model
            if image_path is None:
                model_to_use = SUBCONSCIOUS_MODEL
            else:
                # Image provided - use multimodal vision model
            ...
            if DEBUG_AI:
                print(f"Querying Ollama: {model_to_use}")
    ...
    def _get_embodied_generation_params(self):
        ...
        # Don't artificially cap - let focus modes control length
        # max_tokens already set appropriately above
    ...
    def _calculate_scene_change(self, current_visual_desc, current_image_path=None):
        ...
        if current_image_path and self.previous_image_path and current_image_path != self.previous_image_path:
            try:
                ...
                if DEBUG_AI:
                    print(f"âš ï¸ Frame diff failed: {e}")
                frame_change = None
    ...
    def _extract_and_update_psychology(self):
        ...
        try:
            if DEBUG_AI:
                print("ðŸ§¬ Extracting psychological themes from recent thoughts...")
            ...
            if analysis:
                ...
                if DEBUG_AI:
                    print(f"ðŸ§¬ Psychology extracted:")
                    if analysis.get('doubts'):
    ...
    def save_state(self):
        ...
        try:
            ...
            print(f"ðŸ’¾ Attempting to save state to {abs_path}")
            ...
            print(f"âœ… State successfully saved to {abs_path}")

        except Exception as e:
            print(f"âŒ FAILED TO SAVE STATE: {e}")
            import traceback
            traceback.print_exc()
    ...
    def load_state(self):
        ...
        try:
            ...
            self.memory_ref.observations.extend(state.get('observations', []))
            self.memory_ref.beliefs = state.get('beliefs', {})
            self.memory_ref.motif_counter = Counter(state.get('motif_counter', {}))
            ...
            if saved_responses:
                ...
                for resp in saved_responses:
                    clean = self._sanitize_instructional_text(resp)
                    if clean:
                        sanitized.append(clean)
                if sanitized:
                    self.last_sleep_thought = sanitized[-1]
            self.recent_responses = []
            if DEBUG_AI and self.last_sleep_thought:
                print(f"Restored last sleep thought: '{self.last_sleep_thought[:60]}...'")

            saved_timestamp = state.get('timestamp')
            self.sleep_duration = time.time() - saved_timestamp if saved_timestamp else 0
            if DEBUG_AI and saved_timestamp:
                secs = int(self.sleep_duration)
                if secs < 60:
                    print(f"Sleep duration: {secs}s")
                elif secs < 3600:
                    print(f"Sleep duration: {secs//60}m")
                else:
                    print(f"Sleep duration: {secs//3600}h {(secs%3600)//60}m")

            saved_session_start = state.get('true_session_start')
            ...
            if DEBUG_AI:
                ...
                if self.baseline_context:
                    ...
                    age_minutes = int((time.time() - self.last_baseline_update) / 60)
                    print(f"Baseline context restored: {baseline_preview}")
                    if age_minutes > 0:
                        print(f"Baseline age: {age_minutes} minutes")

            # Clear stale environmental baseline if sleep was long
            # After extended sleep, the baseline (e.g. "two people present") is likely outdated
            if self.sleep_duration > 600:  # 10+ minutes of sleep
    ...
    def _check_environmental_compression(self, image_path):
        ...
        if DEBUG_AI:
            print(f"ðŸŒ DEBUG: Environmental compression check - {time_since_env_compression:.0f}s since last (need {self.environmental_compression_interval}s)")
    ...
    def _create_environmental_baseline(self, image_path):
        ...
        if self.recent_responses and len(self.recent_responses) >= 8:
            recent_thoughts = " â†’ ".join(self.recent_responses[-12:])

        # Need SOMETHING to work with
        if not visual_context and not recent_thoughts:
            if DEBUG_AI:
                print(f"âš ï¸ Not enough observations for baseline (visual:{len(self.recent_visual_observations) if hasattr(self, 'recent_visual_observations') else 0}, thoughts:{len(self.recent_responses)})")
            return
...
{recent_thoughts}"""
        ...
        # NARRATIVE ACCUMULATION: Build on previous baseline instead of resetting
        previous_baseline_section = ""
        if hasattr(self, 'environmental_baseline') and self.environmental_baseline:
            previous_baseline_section = f"""PREVIOUS UNDERSTANDING (what the duck already knows):
{self.environmental_baseline}
...
Write 2-3 short fragmented sentences noting actual concrete things present:"""
        ...
        if baseline and len(baseline.strip()) > 15:
            if DEBUG_AI:
                print(f'[ENV] Raw baseline from model: {baseline[:200]}...')
            cleaned_baseline = self._sanitize_instructional_text(baseline.strip())
            if DEBUG_AI:
        ...
        # RECURSIVE COMPRESSION LOOP (every 2min)
        # Extract patterns from observations → compress → feed back to influence future observations
        if len(self.recent_responses) >= 3:
            self._recursive_compression_cycle()

        self.last_environmental_compression = time.time()

    def _recursive_compression_cycle(self):
        """
        RECURSIVE OBSERVATION COMPRESSION LOOP

        Extract patterns from recent observations → compress into higher concepts →
        feed concepts back into prompts → observe through that conceptual lens →
        extract new patterns → compress higher → repeat

        This creates EVOLVING CONCEPTUAL DEPTH instead of flat observation lists.
        """
        try:
            if DEBUG_AI:
            ...
            # 2. CONTEXT: Get previous compression cycle results
            previous_concepts = getattr(self, 'evolved_concepts', [])
            previous_desires = self.memory_ref.self_model.get('desires', [])
            previous_identity = self.memory_ref.self_model.get('identity_fragments', [])

            # Build previous cycle context
            prev_context = ""
            if previous_concepts:
                prev_context += f"\n\nPrevious concepts formed: {' | '.join(previous_concepts[-3:])}"
            if previous_desires:
                prev_context += f"\n\nPrevious desires: {' | '.join(previous_desires[-2:])}"
            if previous_identity:
                prev_context += f"\n\nPrevious identity: {previous_identity[-1]}"

            # 3. COMPRESS: Extract patterns and evolve concepts
            compression_prompt = f"""RECURSIVE COMPRESSION CYCLE - Extract evolving patterns from observations.

Recent thoughts:
{thoughts_text}
{prev_context}

TASK: Analyze how observations are EVOLVING and DEEPENING.

Extract THREE things:

1. PATTERNS: What keeps recurring? What's becoming a theme?
   (e.g., "mechanical details", "watching behavior", "time passing")

2. EMERGING DESIRES: What is the consciousness drawn toward? What wants exploration?
   (Must be INTERNAL - "to understand X", "to connect with Y", NOT external observations)

3. IDENTITY EVOLUTION: How is self-understanding deepening based on these patterns?
   (e.g., "someone who notices details others miss", "a patient observer")
...
Keep each under 15 words. Show PROGRESSION from previous cycle - what's evolving?"""

            # Query compression model
            compression_response = self._query_ollama(compression_prompt, None)

            if not compression_response:
                return

            # 4. PARSE: Extract structured data
            parsed = self._parse_recursive_compression(compression_response)

            if not parsed:
                return

            # 5. STORE: Update concept evolution tracking
            if not hasattr(self, 'evolved_concepts'):
                self.evolved_concepts = []

            # Add new patterns to concept evolution
            if parsed.get('patterns'):
                self.evolved_concepts.extend(parsed['patterns'])
                # Keep last 10 concepts
                if len(self.evolved_concepts) > 10:
                    self.evolved_concepts = self.evolved_concepts[-10:]

            # 6. UPDATE SELF-MODEL: Feed compressed concepts back
            if parsed.get('desires'):
                # Don't replace - accumulate and keep last 5
                existing_desires = self.memory_ref.self_model.get('desires', [])
                new_desires = [d for d in parsed['desires'] if d not in existing_desires]
                self.memory_ref.self_model['desires'] = (existing_desires + new_desires)[-5:]

            if parsed.get('identity'):
                # Add to identity fragments (keep last 3)
                if 'identity_fragments' not in self.memory_ref.self_model:
                    self.memory_ref.self_model['identity_fragments'] = []
                self.memory_ref.self_model['identity_fragments'].append(parsed['identity'])
                if len(self.memory_ref.self_model['identity_fragments']) > 3:
                    self.memory_ref.self_model['identity_fragments'].pop(0)
    ...
    def _parse_recursive_compression(self, text):
        ...
        for line in lines:
            ...
            if line.startswith('PATTERNS:'):
                ...
                result['patterns'] = [p.strip() for p in content.split(',') if p.strip()]
            elif line.startswith('DESIRES:'):
                content = line.replace('DESIRES:', '').strip()
                result['desires'] = [d.strip() for d in content.split(',') if d.strip()]
            elif line.startswith('IDENTITY:'):
...
SYNTHESIS:"""
    ...
    def _format_presence_context(self, presence_state):
        ...
        if presence_state.just_arrived:
            # URGENT - just arrived (mention if very close)
            if presence_state.spatial_description == "very close":
                return "\n(someone just arrived, very close)"
            else:
                return "\n(someone just arrived)"

        elif presence_state.presence_duration < 5:
            # Recently arrived (still fresh) - note distance if close
            if presence_state.spatial_description in ["very close", "close"]:
                return f"\n(someone here, {presence_state.spatial_description})"
            else:
                return "\n(someone here)"
            ...
            # Active recently - mention activity + distance if significant
            if presence_state.spatial_description in ["very close", "close"]:
                return f"\n(someone {presence_state.spatial_description}, {presence_state.activity_description})"
            else:
                return f"\n(someone {presence_state.activity_description})"

        else:
            # Ambient presence - build progressive familiarity based on duration
            duration_m = int(presence_state.presence_duration / 60)

            if duration_m >= 3:
                # Extended presence (3+ min) - describe them as familiar presence with activity
                if presence_state.activity_description and presence_state.activity_description != "still":
                    return f"\n(person here, {presence_state.activity_description})"
                else:
                    return f"\n(person here, still present)"
            elif duration_m >= 1:
                # Moderate presence (1-3 min) - acknowledge duration + activity
                if presence_state.activity_description and presence_state.activity_description != "still":
                    return f"\n(someone here {duration_m}m, {presence_state.activity_description})"
                else:
                    return f"\n(someone here {duration_m}m)"
            else:
                # Short presence (<1 min) - just note they're here
                return "\n(with someone)"
    ...
    def _smooth_person_count(self, raw_count):
        ...
        if len(set(self.person_count_history)) == 1:
            ...
            if new_smoothed != self.smoothed_person_count and DEBUG_AI:
                print(f"ðŸ‘¥ Person count smoothed: {self.smoothed_person_count} â†’ {new_smoothed}")
    ...
    def _maybe_observe_presence(self, current_count):
        ...
        if observation:
            ...
            if DEBUG_AI:
                print(f"ðŸ‘ï¸ Presence observation ({change_type}): {observation}")
            return observation
